 ...
 * The event lane.
 *
 * POST — one confirmed reading becomes one row.
 * GET  — the recent readings plus the hourly rollup.
 *
 * Neither ever returns an error the capture path has to handle. If there is no
 * database, that is a fact about the deployment, not a failure of the reading:
 * the sentence was still spoken.
 */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Confidence ranks candidates; it is not a probability. Sort, take the top.
    ...
    // Logged, not surfaced. A database that is down is not a reason to tell
    // someone their sentence failed.
...
export async function GET() {
  ...
  try {
    ...
    // The rollup the lab and ward cases both want: how often did it have to
    // ask, by room, by hour. Served from the continuous aggregate when it
    // exists, computed live when it does not.
    let hourly;
